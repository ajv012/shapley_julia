...
function gsa(f, method::Shapley, input_distribution::SklarDist;batch=false)
    ...
    Threads.@threads for i in range(2, n_boot)
    ...
    ξ[:, 2:end, :, :]  .= ξ[:, 2:end, :, :] - ξ[:, 1:end-1, :, :]; # check on whether done correctly in julia

    # Cost variation
    Threads.@threads for i in range(1, n_boot)
